...
router.get('/my-books', authMiddleware, async (req, res) => {
  try {
    ...
    // Attach buyer count to each book
...
router.post('/upload-pdf', authMiddleware, uploadPdf.single('pdfFile'), async (req, res) => {
  try {
    ...
    // Save original file extension so download/stream can use correct type
    const originalName = req.file.originalname || '';
    const origExt = originalName.split('.').pop()?.toLowerCase();
    book.pdfFileExt = (origExt === 'docx' || origExt === 'doc') ? origExt : 'pdf';
    await book.save();
