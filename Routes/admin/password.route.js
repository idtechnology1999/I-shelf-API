...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    await PasswordReset.deleteMany({ email, userType: 'admin' });
    ...
    const mailOptions = {
      from: '"iShelf Admin" <' + EMAIL_USER + '>',
      to: email,
      subject: 'Admin Password Reset Code - iShelf',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background-color: #E85D54; padding: 20px; text-align: center;">
            <h1 style="color: white; margin: 0;">iShelf Admin</h1>
          </div>
          <div style="padding: 30px; background-color: #f9f9f9;">
            <h2 style="color: #333;">Password Reset Request</h2>
            <p style="color: #666; font-size: 16px;">Hello ${admin.fullName},</p>
    ...
    console.log(`Admin password reset code sent to ${email}: ${code}`);
...
router.post('/reset-password', async (req, res) => {
  ...
  try {
    ...
    await Admin.findOneAndUpdate(
      { email },
      { password: hashedPassword, isSetupComplete: true },
